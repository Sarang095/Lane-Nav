...
def train_rl_agent(
    ...
    """
    Train PPO agent for highway-v0 environment.
    ...
    Returns:
        Trained PPO agent
    """
    print("Training PPO agent for highway-v0...")
    ...
    eval_env = create_environment("highway")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="highway_checkpoint"
    )
    ...
    if save_path is None:
        save_path = os.path.join(
            ...
            get_model_filename("highway", "rl", "ppo")
        )
...
def train_il_agent(
    ...
    """
    Train Imitation Learning agent for highway-v0 environment.
    ...
    """
    print(f"Training {algorithm.upper()} agent for highway-v0...")
    ...
    if algorithm.lower() == "bc":
        ...
        # Train BC agent
        trainer = train_behavioral_cloning("highway", demonstrations)
        
    elif algorithm.lower() == "dagger":
        # Train DAgger agent
        trainer = train_dagger("highway", expert_model)
        
    else:
        raise ValueError(f"Unknown algorithm: {algorithm}. Use 'bc' or 'dagger'")
    
    # Save trained model
    model_path = save_imitation_model(trainer, "highway", algorithm.lower())
    return model_path


def main():
    """Main training function with command line interface."""
    parser = argparse.ArgumentParser(description="Train autonomous driving agent for highway-v0")
    parser.add_argument(
        "--mode", 
        type=str, 
        choices=["rl", "il"], 
    ...
    print(f"python evaluate.py highway --mode {args.mode}")
