...
def train_rl_agent(
    ...
    """
    Train PPO agent for roundabout-v0 environment.
    ...
    Returns:
        Trained PPO agent
    """
    print("Training PPO agent for roundabout-v0...")
    
    # Create environment
    env = create_environment("roundabout")
    env = Monitor(env, TRAINING_CONFIG["log_dir"])
    
    # Create evaluation environment
    eval_env = create_environment("roundabout")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="roundabout_checkpoint"
    )
    ...
    if save_path is None:
        save_path = os.path.join(
            ...
            get_model_filename("roundabout", "rl", "ppo")
        )
...
def train_il_agent(
    ...
    """
    Train Imitation Learning agent for roundabout-v0 environment.
    ...
    """
    print(f"Training {algorithm.upper()} agent for roundabout-v0...")
    ...
    if algorithm.lower() == "bc":
        ...
        # Train BC agent
        trainer = train_behavioral_cloning("roundabout", demonstrations)
        
    elif algorithm.lower() == "dagger":
        # Train DAgger agent
        trainer = train_dagger("roundabout", expert_model)
        
    else:
        raise ValueError(f"Unknown algorithm: {algorithm}. Use 'bc' or 'dagger'")
    
    # Save trained model
    model_path = save_imitation_model(trainer, "roundabout", algorithm.lower())
    return model_path


def main():
    """Main training function with command line interface."""
    parser = argparse.ArgumentParser(description="Train autonomous driving agent for roundabout-v0")
    parser.add_argument(
        "--mode", 
        type=str, 
        choices=["rl", "il"], 
    ...
    print(f"python evaluate.py roundabout --mode {args.mode}")
