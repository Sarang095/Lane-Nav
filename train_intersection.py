...
def train_rl_agent(
    ...
    """
    Train PPO agent for intersection-v0 environment.
    ...
    Returns:
        Trained PPO agent
    """
    print("Training PPO agent for intersection-v0...")
    
    # Create environment
    env = create_environment("intersection")
    env = Monitor(env, TRAINING_CONFIG["log_dir"])
    
    # Create evaluation environment
    eval_env = create_environment("intersection")
    eval_env = Monitor(eval_env, TRAINING_CONFIG["log_dir"] + "/eval")
    
    # Initialize PPO agent
    model = PPO(
        "MlpPolicy",  # Use MLP policy for Kinematics observations
        env,
        **PPO_HYPERPARAMS
    )
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="intersection_checkpoint"
    )
    ...
    if save_path is None:
        save_path = os.path.join(
            ...
            get_model_filename("intersection", "rl", "ppo")
        )
...
def train_il_agent(
    ...
    """
    Train Imitation Learning agent for intersection-v0 environment.
    ...
    """
    print(f"Training {algorithm.upper()} agent for intersection-v0...")
    ...
    if algorithm.lower() == "bc":
        ...
        # Train BC agent
        trainer = train_behavioral_cloning("intersection", demonstrations)
        
    elif algorithm.lower() == "dagger":
        # Train DAgger agent
        trainer = train_dagger("intersection", expert_model)
        
    else:
        raise ValueError(f"Unknown algorithm: {algorithm}. Use 'bc' or 'dagger'")
    
    # Save trained model
    model_path = save_imitation_model(trainer, "intersection", algorithm.lower())
    return model_path


def main():
    """Main training function with command line interface."""
    parser = argparse.ArgumentParser(description="Train autonomous driving agent for intersection-v0")
    parser.add_argument(
        "--mode", 
        type=str, 
        choices=["rl", "il"], 
    ...
    print(f"python evaluate.py intersection --mode {args.mode}")
